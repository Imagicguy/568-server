...
private:
  ...
  void print_error(int &ID, int status) {
    ...
    if (status == 400) {
      ...
      file << ID << " : "
           << "Responding HTTP/1.1 502 Bad Gateway\n";
      file.close();
      return;
    } else if (status == 411) {
      file << ID << " : "
           << "ERROR invalid POST Require header\n";
      file << ID << " : "
           << "Responding HTTP/1.1 411 Length Required\n";
      file.close();
      return;
    } else if (status == -100) {
      file << ID << " : "
  ...
  void get_requestHelper(std::unordered_map<std::string, Http_Response> &cache,
                         int &ID) {
    std::time_t result = std::time(NULL); // get current time
    std::string temp;
    int recv_len;
    if (!cache.empty()) {
      Http_Response a = cache.at(0);

      std::cout << "cache data is " << a.data.data() << std::endl;
    }
    do {
    ...
    if (P.method_send() == "CONNECT") {
      ...
      if (cache.find(req) != cache.end()) {
        ...
        if ((temp.find("no-cache") != std::string::npos) ||
            (cache[req].revalidation)) {
          std::ofstream file;
          file.open("/var/log/erss/proxy.log", std::ios::app);
          file << ID << " : "
  ...
  void handle_connect(std::string orghost, int P, int &ID) {
    std::cout << "now in handle_request" << std::endl;
    std::cout << "orghost is " << orghost << std::endl;
    ...
    int response =
        connect(origin_fd, (struct sockaddr *)&origin, sizeof(origin));
    if (response < 0) {
      print_error(ID, -400);
      close(origin_fd);
      return;
    ...
    int highest =
        (origin_fd > client_connection_fd) ? origin_fd : client_connection_fd;
    fd_set readfds;
    while (1) {
      FD_ZERO(&readfds);
  ...
  /***********simple recvive function only to recvive response
   * header*****************/
  ...
  void
  simple_receive_helper(std::string res, std::string orghost, int P,
  ...
  void
  handle_requestHelper(std::string res, std::string orghost, int P,
                       std::unordered_map<std::string, Http_Response> &cache,
                       int &ID) {
    std::string bb = res;
    ...
    file.open("/var/log/erss/proxy.log", std::ios::app);
    file << ID << " : "
         << "Received " << P_res.resp_recv() << " from " << bb << "\n";
    file.close();
    if (!((P_res.control_recv()).empty())) {
      file.open("/var/log/erss/proxy.log", std::ios::app);
      file << ID << " : "
           << "NOTE " << P_res.control_recv() << "\n";
      file.close();
    }
    if (!((P_res.Etag_recv()).empty())) {
      file.open("/var/log/erss/proxy.log", std::ios::app);
      file << ID << " : "
           << "NOTE " << P_res.Etag_recv() << "\n";
      file.close();
    }
    SaveCache(res, target, newbuff, cache, ID);
    file.open("/var/log/erss/proxy.log", std::ios::app);
    file << ID << " : "
         << "Responding " << P_res.resp_recv() << "\n";
    file.close();
    int actual_send;
  ...
  void handle_chunk(std::vector<char> &newbuff, int origin_fd, int former_recv,
                    int ID) {
    try {
...
public:
  /***********constructor**************/
  Proxy(int s, int c, std::string h)
      : socket_fd(s), client_connection_fd(c), hostinfo(h) {}
  /************************************/
