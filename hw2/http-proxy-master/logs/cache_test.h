...
void SaveCache(std::string req, int size, std::vector<char> response,
               std::unordered_map<std::string, Http_Response> &cache, int ID) {
  ...
  if (!P_res.status_recv()) {
    ...
    file << ID << " : "
         << "not cacheable because not receive 200 OK\n";
    file.close();
    return;
  }
  if (!(P_res.control_recv()).empty()) {
    std::string control = P_res.control_recv();
    if ((control.find("no-cache") != std::string::npos) ||
        (control.find("no-store") != std::string::npos)) {
      std::ofstream file;
      file.open("/var/log/erss/proxy.log", std::ios::app);
      file << ID << " : "
           << "not cacheable because " << control << "\n";
      file.close();
      return;
    }
  }
  if (size > INT_MAX) {
    ...
    file.open("/var/log/erss/proxy.log", std::ios::app);
    file << ID << " : "
         << "not cacheable because this web is too large\n";
    file.close();
    return;
  }
  if (cache.size() > 100) {
    ...
    file << "(no-id)"
         << " : "
         << "evicted " << bb
         << "from cache because cache can only store 100 pages\n";
    file.close();
  ...
  if (!(P_res.control_recv()).empty()) {
    ...
    if (control.find("must-revalidate") != std::string::npos) {
      ...
      file.open("/var/log/erss/proxy.log", std::ios::app);
      file << ID << " : "
           << "cached, but requires revalidation\n";
      file.close();
  ...
  if (!(P_res.expire_recv()).empty()) {
    ...
    file.open("/var/log/erss/proxy.log", std::ios::app);
    file << ID << " : "
         << "cached, but expires at " << asctime(&e);
    file.close();
...
int SearchCache(std::string req, std::vector<char> response,
                std::unordered_map<std::string, Http_Response> &cache, int fd,
                int ID) { //,int fd
    ...
    if (expire_time.empty()) {
      ...
      if (modify_time.empty()) {
        ...
        if (diff_2 > 0) {
          ...
          file.open("/var/log/erss/proxy.log", std::ios::app);
          file << ID << " : "
               << "in cache, but modified at " << asctime(&m) << "\n";
          file.close();
          get_info = 1;
          return get_info;
        }
      ...
      if (diff_1 < 0) {
        ...
        file.open("/var/log/erss/proxy.log", std::ios::app);
        file << ID << " : "
             << "in cache, but expires at " << asctime(&e);
        file.close();
        get_info = 1;
        return get_info;
      }
      ...
      if (diff_1 > 0) {
        ...
        if (modify_time.empty()) { // modify time does not exist, send back
          ...
          if (diff_2 > 0) {
            ...
            file.open("/var/log/erss/proxy.log", std::ios::app);
            file << ID << " : "
                 << "in cach, but modified at " << modify_time << "\n";
            file.close();
            get_info = 1;
            return get_info;
          }
